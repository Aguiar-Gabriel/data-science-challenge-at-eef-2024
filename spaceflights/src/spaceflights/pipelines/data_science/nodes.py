...
def train_model(
    ...
    X_tr_numeric = X_tr[numeric_cols]
    X_val_numeric = X_val[numeric_cols]

    # Impute remaining NaNs with the median
    for col in numeric_cols:
        if X_tr_numeric[col].isnull().any():
            median_val = X_tr_numeric[col].median()
            X_tr_numeric.loc[:, col] = X_tr_numeric.loc[:, col].fillna(median_val)
            X_val_numeric.loc[:, col] = X_val_numeric.loc[:, col].fillna(median_val)

    if use_smote:
        logger.info("Aplicando SMOTE para balanceamento de classes.")
        sm = SMOTE(random_state=42)
        X_tr_res, y_tr_res = sm.fit_resample(X_tr_numeric, y_tr)
    else:
        X_tr_res, y_tr_res = X_tr_numeric, y_tr
    ...
    if "catboost" in model_type.lower():
        ...
        for col in cat_features:
            ...
            X_val_numeric[col] = X_val_numeric[col].astype('category')

        model.fit(X_tr_res, y_tr_res, cat_features=cat_features)
    else:
        model.fit(X_tr_res, y_tr_res)

    y_pred_val = model.predict(X_val_numeric)
...
def predict_espera(
    ...
    numeric_cols = X_to_predict.select_dtypes(include=np.number).columns.tolist()
    X_to_predict_numeric = X_to_predict[numeric_cols]

    return model.predict(X_to_predict_numeric)
